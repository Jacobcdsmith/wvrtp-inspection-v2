...
const INSPECTOR_KEY = "wvrtp_inspector_name";

export function saveToHistory(record: Record<string, unknown>) {
  try {
...
export default function InspectPage() {
  ...
  const [inspectorName, setInspectorName]     = useState(
    () => localStorage.getItem(INSPECTOR_KEY) || ""
  );
  ...
  // Persist inspector name so staff don't retype it every submission
  const handleInspectorNameChange = (value: string) => {
    setInspectorName(value);
    try { localStorage.setItem(INSPECTOR_KEY, value); } catch { /* non-critical */ }
  };
  ...
  const validateChecklist = (): string | null => {
    ...
    const fields = CHECKLISTS[equipmentType as EquipmentType];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (checklistError) {
      toast({ title: "Checklist Incomplete", description: checklistError, variant: "destructive" });
      return;
    }
    ...
    if (webhookUrl) {
      try {
        ...
        toast({
          ...
          description: `Inspection saved locally. Webhook failed — ${msg}. Will not retry.`,
          variant: "destructive",
        });
        setSubmitting(false);
        return;
    ...
    // Reset form (keep inspector name — they're still the same person)
    setEquipmentId(""); setIsFromQr(false); setEquipmentType("");
    setChecklistValues({}); setOverallResult("");
                ...
                <div key={field.key} className="space-y-1.5">
                  ...
                  {field.type === "number" ? (
                    <input
                      ...
                      placeholder={`Enter ${field.label.toLowerCase()}${field.unit ? ` (${field.unit})` : ""}`}
